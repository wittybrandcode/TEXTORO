// TEXTORO Typewriter sourceText Expression v1.4
// v1.3: النص من Marker (TW_TEXT:)  |  v1.4 NEW: Live Text
// ═══════════════════════════════════════════════════════════════════
// v1.4 Updates:
// - Live Text Mode: قراءة النص مباشرة من الطبقة (value.text)
//   * تعديل النص من AE أو من Premiere Essential Graphics ينعكس فوراً
//   * متوافق مع تصدير Mogrt - النص قابل للتحرير بدون أي لوحة
//   * عند إيقاف Live Text يعود لقراءة TW_TEXT marker (وضع مجمّد)
// - Fallback آمن: غياب كنترول "Live Text" = وضع Marker كما في v1.3
// ═══════════════════════════════════════════════════════════════════
...
var L;
// ═══════════════════════════════════════════════════════════════════
// v1.4 LIVE TEXT: قراءة النص الحي من الطبقة نفسها
// value داخل تعبير الخاصية = القيمة الثابتة قبل التعبير (نص المستخدم)
// ═══════════════════════════════════════════════════════════════════
var liveMode = 0;
try { liveMode = effect("Live Text")(1); } catch (e) { liveMode = 0; }
if (liveMode == 1) {
    try {
        var __doc = value;                       // TextDocument قبل التعبير
        X = (typeof __doc === "string") ? __doc : String(__doc.text);
    } catch (e) { /* fallback: يبقى X من الـ marker */ }
}
L = X.length;
